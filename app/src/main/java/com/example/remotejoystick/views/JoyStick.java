...
public class JoyStick extends SurfaceView implements SurfaceHolder.Callback, View.OnTouchListener
{
    ...
    public void setListener(JoystickListener jsl){
        this.joystickCallback = jsl;
    }
    private void drawJoystick(float newX, float newY)
    {
        if(getHolder().getSurface().isValid())
        {
            ...
            colors.setARGB(255, 184, 191, 194);
            myCanvas.drawCircle(centerX, centerY, baseRadius, colors);

            //Drawing the joystick hat
            for(int i = 0; i <= (int) (hatRadius / ratio); i++)
            {
               // colors.setARGB(100, (int) (i * (100 * ratio / hatRadius)), (int) (i * (100 * ratio / hatRadius)), 100);
                colors.setARGB(150, 50, 13, 75);//Change the joystick color for shading purposes
